...
// Close modals/dropups when double-clicking off within 1s
let lastClick = 0;
...
function saveLB(){ localStorage.setItem(LS_KEY, JSON.stringify(LEADERBOARD)); }
function renderLB(){
...
function ensureTeamSelect(){
  ...
  sel.value = localStorage.getItem('player_team') || '0';
...
// Called by game.js when the USER (team A) scores a try (5 pts) or other points
window.__onUserScore = function(points){
  const id = parseInt(localStorage.getItem('player_team') || '0', 10);
  if(!isNaN(id) && LEADERBOARD[id]){
    LEADERBOARD[id].pts += points;
    saveLB(); renderLB();
  }
};

// ----- League standings (placeholder table to look like site) -----
...
// ----- Profile: followers/following/friends dropdowns and avatar modal -----
if(document.title.includes('Profile')){
  ...
  for(let i=1;i<=32;i++){
    ...
    b.addEventListener('click', ()=>{
      // simple generated avatar tile
      const canvas = document.createElement('canvas'); canvas.width=256; canvas.height=256;
      const x = canvas.getContext('2d'); x.fillStyle = `hsl(${(i*36)%360} 70% 45%)`; x.beginPath(); x.arc(128,128,120,0,Math.PI*2); x.fill();
